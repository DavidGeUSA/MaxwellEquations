...
TaskFile::TaskFile(const char *taskfile)
{
	...
	ret = ERR_OK;
	//printf("\n FILENAME_MAX=%u \n", FILENAME_MAX); std::cout << std::flush;
	strcpy(filepath, taskfile);
	//puts("\n 2 \n"); std::cout << std::flush;
	isDisabled = false;
	unfoundName = NULL;
	nameBadVal = NULL;
	...
	if (_lines != NULL)
	{
		...
		if (ret == ERR_OK)
		{
			...
			else
			{
				ret = writefile(fh, "//", 2);
				if (ret == ERR_OK)
					ret = writefile(fh, filepath, (unsigned int)strlen(filepath));
				if (ret == ERR_OK)
					ret = writefile(fh, "\r\n", 2);
				if (ret == ERR_OK)
				{
...
bool TaskFile::hasMissingName()
{
	return (unfoundName != NULL);
}
void TaskFile::showMissingName()
{
	if (unfoundName != NULL)
	{
		printf("%s", unfoundName);
	}
}
bool TaskFile::hasInvalidName()
{
	return (nameBadVal != NULL);
}
void TaskFile::showInvalidName()
{
	if (nameBadVal != NULL)
	{
		printf("%s", nameBadVal);
	...
	if (ret == ERR_OK)
	{
		...
		if (ret == ERR_OK)
		{
			if (n >= 0)
			{
				...
				{
					char *q = strchr(_lines[n].value, '/'); //support division
					if (q == NULL)
					{
...
int TaskFile::setString(const char *name, const char *value)
{
	int n = getIndexByName(name);
	if (ret == ERR_TASK_PARAMETER_NAME)
	{
	...
	if (ret == ERR_OK)
	{
		if (n >= 0)
		{
			...
			_lines[n].value = (char *)malloc(strlen(value)+1);
			if (_lines[n].value == NULL)
			{
				ret = ERR_OUTOFMEMORY;
			}
		...
		else
		{
			...
			_lines[count].name = (char *)malloc(strlen(name) + 1);
			if (_lines[count].name == NULL)
			{
				ret = ERR_OUTOFMEMORY;
			}
			...
			{
				_lines[count].value = (char *)malloc(strlen(value) + 1);
				if (_lines[count].value == NULL)
				{
					free(_lines[count].name); _lines[count].name = NULL;
					ret = ERR_OUTOFMEMORY;
				}
				else
				{
					...
					{
						_lines[count].name[i] = name[i];
					}
					_lines[count].name[i] = 0;
					for (i = 0; i < strlen(value); i++)
					{
						_lines[count].value[i] = value[i];
					}
					_lines[count].value[i] = 0;
					count++;
				}
			}
		}
...
int TaskFile::getStringVector(const char *name, bool optional, vector<string> &v)
{
	if (ret == ERR_OK)
	{
		int n = getIndexByName(name);
...
void doubleToIntegers(double d, int *i1, int *i2, int *en)
{
	...
	double eps = 1e-10;
	bool gotDigit = false;
	//make sure d is a "well-formed" value
	*en = 0;
	if (d < 0)
	{
		d = -d;
...
int doubleToIntegersString(double d, char *str, unsigned int size)
{
	...
	if (en > 0)
	{
		if (i2 == 0)
		{
			int ret = sprintf_1(str, size, "%de%d", i1, en) <= 0 ? ERR_STR_PRF : ERR_OK;
			if (ret != ERR_OK)
				return ret;
		}
		else
		{
			int ret = sprintf_1(str, size, "%de%dD%d", i1, en, i2) <= 0 ? ERR_STR_PRF : ERR_OK;
			if (ret != ERR_OK)
				return ret;
		}
	...
	else
	{
		if (i2 == 0)
		{
			int ret = sprintf_1(str, size, "%d", i1) <= 0 ? ERR_STR_PRF : ERR_OK;
			if (ret != ERR_OK)
				return ret;
		}
		else
