...
private:
	...
	unsigned long lastError;
	//
	void *mapFileIntoMemory(const char *filepath, size_t *size, int *nRet, bool createFile);
public:
	...
	//
	void *ReadFileIntoMemoryItem(const char *filepath, size_t *size, int *nRet);
	void *CreateFileIntoMemoryItem(const char *filepath, size_t size, int *nRet);
