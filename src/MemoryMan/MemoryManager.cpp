...
int MemoryItem::ReadFileIntoMemoryItem(size_t *size)
{
	...
	if(ret == ERR_OK)
	{
		...
		addr = mmap(NULL, length + offset - pa_offset, PROT_READ,
				MAP_SHARED/*MAP_PRIVATE*/, fd, pa_offset);
		if (addr == MAP_FAILED)
			ret = ERR_FILE_MAP;
	}
	if(ret == ERR_OK)
...
void *MemoryManager::mapFileIntoMemory(const char *filepath, size_t *size, int *nRet, bool createFile)
{
	int i;
	int ret;
	bool full = true;
...
void *MemoryManager::ReadFileIntoMemoryItem(const char *filepath, size_t *size, int *nRet)
{
	return mapFileIntoMemory(filepath, size, nRet, false);
}
void *MemoryManager::CreateFileIntoMemoryItem(const char *filepath, size_t size, int *nRet)
{
